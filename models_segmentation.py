...
class FCN():
    def __init__(self, front_end, stride, num_classes, is_training):
        ...
        # (deconvolution) layer.
        input_shape = tf.shape(self.front.X_input, name='shape_input')[1:3]

        self.score = get_conv('score', self.front.net, pretrained=False, bias=True, k_size=[1, 1],
        ...
        if stride == 32:
            ...
            f16_shape = tf.shape(self.front.f16, name='shape_f16')[1:3]
            ...
            if stride == 16:
                ...
                f8_shape = tf.shape(self.front.f8, name='shape_f8')[1:3]
...
class UNet():
    def __init__(self, front_end, num_classes, is_training):
        ...
        get_conv = lambda name, bottom, k_size, num_outputs: \
            self.front.cc.get_conv_bn(name, bottom, is_training=is_training, k_size=(1, 1), num_outputs=1)

        with tf.variable_scope('unpool_block2'):
            self.unpool2 = tf.concat(
                [self.front.f16, tf.image.resize_bilinear(self.front.f32, tf.shape(self.front.f16)[1:3])], -1)
        ...
        with tf.variable_scope('unpool_block3'):
            self.unpool3 = tf.concat(
                [self.front.f8, tf.image.resize_bilinear(self.u_conv2_2, tf.shape(self.front.f8)[1:3])], -1)
        ...
        with tf.variable_scope('unpool_block4'):
            self.unpool4 = tf.concat(
                [self.front.f4, tf.image.resize_bilinear(self.u_conv3_2, tf.shape(self.front.f4)[1:3])], -1)
        ...
        with tf.variable_scope('unpool_block5'):
            self.unpool2 = tf.concat(
                [self.front.f2, tf.image.resize_bilinear(self.u_conv4_2, tf.shape(self.front.f2)[1:3])], -1)
        ...
        with tf.variable_scope('score'):
            self.score = tf.image.resize_bilinear(self.u_conv5_2, tf.shape(self.front.X_input)[1:3])
...
class PSPNet():
    def __init__(self, front_end, num_classes, is_training=True):
        """
        :param stride: specify the stride of the FCN network (FCN#{stride}s).
        :param ignore: If there exists at least one label (class) to be ignored when calculating the loss.
        """
        ...
        get_bn = lambda name, bottom: self.front.cc.get_bn(name, bottom, momentum=deeplab_batch_norm_decay,
...
class Deeplabv3():
    def __init__(self, front_end, num_classes, is_training, bn_scale):
        self.front = front_end
        input_shape = tf.shape(self.front.X_input)[1:3]
        f32_shape = tf.shape(self.front.f32)[1:3]
        ...
        with tf.variable_scope('ASPP'):
            ...
            # ASPP
            self.ASPP1 = get_conv('ASPP1', self.front.f32, pretrained=False, k_size=[1, 1], num_outputs=256)
            self.ASPP2 = get_conv('ASPP2', self.front.f32, pretrained=False, k_size=[3, 3], num_outputs=256,
                                  atrous=6)
            self.ASPP3 = get_conv('ASPP3', self.front.f32, pretrained=False, k_size=[3, 3], num_outputs=256,
                                  atrous=12)
            self.ASPP4 = get_conv('ASPP4', self.front.f32, pretrained=False, k_size=[3, 3], num_outputs=256,
                                  atrous=18)
            # BN
...
class Deeplabv3p():
    def __init__(self, front_end, num_classes, is_training, bn_scale):
        ...
        input_shape = tf.shape(self.front.X_input)[1:3]
        f32_shape = tf.shape(self.front.f32)[1:3]
        f4_shape = tf.shape(self.front.f4)[1:3]
        ...
        with tf.variable_scope('low_level'):
            stride4 = get_conv('stride_4', self.front.f4, pretrained=False, k_size=[1, 1], num_outputs=48)
        ...
        with tf.variable_scope('ASPP_reduce'):
            ...
            stride4_fuse = get_conv('stride_4_fuser_conv2', stride4_fuse, pretrained=False, k_size=[3, 3],
                                    num_outputs=256)
            stride4_fuse = get_bn('stride_4_fuser_conv2_bn', stride4_fuse)
            self.reduce = get_conv('final_reduce', stride4_fuse, pretrained=False, bias=False, k_size=[1, 1],
                                   num_outputs=num_classes)
            # upsample
            self.logits = tf.image.resize_bilinear(self.reduce, input_shape, name='logits')
...
class SimpleDeconv():
    def __init__(self, front_end, num_classes, is_training, bn_scale, fuse):
        logger.info(f'Simple Deconv with fuse: {fuse}')
        self.front = front_end
        input_shape = tf.shape(self.front.X_input)[1:3]
        f2_shape = tf.shape(self.front.f2)[1:3]
        f4_shape = tf.shape(self.front.f4)[1:3]
        f8_shape = tf.shape(self.front.f8)[1:3]
        ...
        with tf.variable_scope('Simple_Deconv'):
            self.unpool1 = tf.image.resize_bilinear(self.front.net, f8_shape, name='unpool1')
            if 8 in fuse:
                logger.info('Simple Deconv: fuse 8')
                self.unpool1 = tf.concat([self.unpool1, self.front.f8], axis=-1, name='concat_unpool1')
            ...
            if 4 in fuse:
                ...
                self.unpool2 = tf.concat([self.unpool2, self.front.f4], axis=-1, name='concat_unpool2')
            ...
            if 2 in fuse:
                ...
                self.unpool3 = tf.concat([self.unpool3, self.front.f2], axis=-1, name='concat_unpool3')

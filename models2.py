...
class PSPNet():
    def __init__(self, front_end, num_classes, ignore=True):
        ...
        get_variance = self.front.cc.get_variance
        get_pspv = self.front.cc.get_variance_for_all
        self.num_classes = num_classes
        ...
        with tf.variable_scope('PSP_v_module'):
            self.PSPV1 = get_pspv('pspv1', self.front.pool5, (ps[0], ps[0]))
            print(self.PSPV1.shape)
            self.PSPV2 = get_pspv('pspv2', self.front.pool5, (ps[1], ps[1]))
            print(self.PSPV2.shape)           
            self.PSPV3 = get_pspv('pspv3', self.front.pool5, (ps[2], ps[2]))
            print(self.PSPV3.shape)
            self.PSPV4 = get_pspv('pspv4', self.front.pool5, (ps[3], ps[3]))
            print(self.PSPV4.shape)
        
        with tf.variable_scope('PSP_module'):
            #self.variance = upsample('variance',self.get_variance, output_shape=(ps[0], ps[0]))
            ...
            #self.get_variance = get_variance(self.front.pool5, name='variance_pool')
            #print(self.get_variance.shape)
            #self.get_variance = tf.expand_dims(tf.expand_dims(self.get_variance, 1), 1)
            #print(self.get_variance.shape)
            #self.get_variance = tf.reshape(self.get_variance, [self.get_variance.shape[0], ps[0], ps[0], self.get_variance.shape[3]], 'variance')
            #self.get_variance = upsample(bottom=self.get_variance, name='variance', factor=1, output_shape = (ps[0], ps[0]))
            #print(self.get_variance.shape)
            #self.get_variance = get_conv(name = 'variance_256', bottom = self.get_variance, pretrained = False, num_output = 256)
        ...
        with tf.variable_scope('post_stage'):
            ...
            #self.concat = tf.concat([self.concat, self.get_variance], axis = 3, name = 'PSP_variance_concat')
            self.concat = tf.concat([self.concat, self.PSPV1, self.PSPV2, self.PSPV3, self.PSPV4, ], axis = 3, name = 'PSP_variance_concat')
        ...
        '''
        with tf.variable_scope('v_post_stage'):
            self.vconcat = tf.concat([self.front.pool5, self.PSPV1, self.PSPV2, self.PSPV3, self.PSPV4], -1, 'psp_vconcat')
            self.post_conv = get_conv('post_conv', self.vconcat, False, k_size=(3, 3), num_output=nr_f)
            self.post_bn = get_bn('post_bn', self.post_conv, False)
            self.post_dropout = tf.nn.dropout(self.post_bn, 0.9)
            self.reduce = get_conv('reduce', self.post_dropout, pretrained=False, bias=False, k_size=[1, 1],
                                   num_output=num_classes)
...
class deeplab_v3_ResNet50():
    def __init__(self, front_end, num_classes, ignore = True):
        ...
        with tf.variable_scope('ASPP'):
            ...
            self.fc_image_pooling = upsample('image_pooling_size60', self.fc_image_pooling, 60)
            # ASPP
            ...
            '''
            self.fc = self.fc_c0 + self.fc_c1 + self.fc_c2 + self.fc_c3 + self.fc_image_pooling
            '''
        ...
        # to be modified
        self.raw_output = self.fc
        # Predictions: ignoring all predictions with labels greater or equal than n_classes
        self.raw_prediction = tf.reshape(self.raw_output, [-1, num_classes])
        # label_proc = prepare_labels(label_batch, tf.stack(raw_output.get_shape()[1:3]), num_classes=args.num_classes, one_hot=False)

        self.raw_gt = tf.reshape(self.y_input, [-1])
        indices = tf.squeeze(tf.where(tf.less_equal(self.raw_gt, num_classes - 1)))
        self.gt = tf.cast(tf.gather(self.raw_gt, indices), tf.int32)
        self.prediction = tf.gather(self.raw_prediction, indices)

        self.y_pred = tf.argmax(self.score, axis=-1, output_type=tf.int32, name='y_pred')
        self.cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.prediction, labels=self.gt)
